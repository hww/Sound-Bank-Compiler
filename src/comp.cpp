...
short bankCount;						// Число банков
int	  envdiv;							// Насколько делить время в МКС чтобы получать единицы огибающей
...
#endif
	p->revision		= S16((unsigned short)(('B'<<8) + '1'));	// Версия файла
	p->bankCount	= S16(1);									// Число банков = 1
...
int				 pointnum=0;
	...
	add_2_list( ENVELOPE, val, pos);			// Добавили запись в лист
	NEXT(sizeof(ALEnvelopeTable));					// Теперь указываем на обьект
	
	p->type			=   S16(0);
	p->sustaneStart	=	S16(0);					// Установка параметров default
	p->sustaneEnd	=	S16(0);
	p->loopStart	=	S16(0);
	p->loopEnd		=	S16(0);          
	p->pointCount	=	S16(0);
...
loop:
	...
	if	(IS("sustain"))
	{
		...
		p->sustaneStart	= VU16;
		p->sustaneEnd	= S16(s16val(val1));
	}
	else if	(IS("loop"))
	{
		p->type |= U16(AL_ENV_LOOP);
		p->loopStart	= VU16;
		p->loopEnd		= S16(s16val(val1));
	}
	else if	(IS("p"))
	{
		p->pointArray[pointnum].val  = VUC;	
		p->pointArray[pointnum].time = S16(s16val(val1));	
		pointnum++;
		NEXT(sizeof(s32));	
	}
	else if	(IS("end"))
	{
		p->pointArray[pointnum-1].time = S16(0);	
...
void add_envelope(char *val )
{

ALEnvelope *p;

	p = (ALEnvelope*) CURPTR;	
	add_2_list( ENVELOPEOLD, val, pos);			// Добавили запись в лист
	NEXT(sizeof(ALEnvelope));					// Теперь указываем на обьект

	p->attackTime	=	S32(0);					// Установка параметров default
	p->attackVolume	=	UC(AL_VOL_FULL);
	p->decayTime	=	S32(0);
	p->decayVolume	=	UC(AL_VOL_FULL);          
	p->releaseTime	=	S32(0);
...
loop:
	...
	if(get_params(var,val,val1) == 0) return;
	     ...
	     if (IS("attacktime"))		p->attackTime	= VS32;
	else if	(IS("attackvolume"))	p->attackVolume	= VUC;
	else if	(IS("decaytime"))		p->decayTime	= VS32;
	else if	(IS("decayvolume"))		p->decayVolume	= VUC;
	else if	(IS("releasetime")) 	p->releaseTime	= VS32;
...
ALKeyMap *p;
	...
	p = (ALKeyMap*) CURPTR;	
	add_2_list( KEYMAP, val, pos );		// Добавили запись в лист
	NEXT(sizeof(ALKeyMap));				// Теперь указываем на обьект
...
loop:
	...
	if(get_params(var,val,val1) == 0) return;
	     ...
	     if (IS("velmin"))		p->velocityMin	= VUC;
	else if	(IS("velmax"))		p->velocityMax	= VUC;
	else if	(IS("keymin"))		p->keyMin		= VN;
	else if	(IS("keymax"))		p->keyMax		= VN;
...
loop:
	...
	if(get_params(var,val,val1) == 0) return;
         ...
         else            
         {
            ...
            add_2_list( WAVE, var, pos );   // Добавили запись в лист
            NEXT(16);                       // Теперь указываем на обьект sizeof(ALWaveTable)
                ...
                if(LoadWav(val))
                {
                    ...
                    if(write_tbl( wh.handle, wh.dSize))
                    {
                        ...
                        {
                            p->flags    = UC(AL_SF_LOOP);           //wh.Loops[0].dwPlayCount;      /* offset/address flags */
                            p->waveInfo.rawWave.loop=PTR((ALRawLoop*)PTCTL(pos));
                        
                            loop = (ALRawLoop*)CURPTR;
                            NEXT(sizeof(ALRawLoop));

                            loop->start = U32(wh.Loops[0].dwStart); 
                            loop->end   = U32(wh.Loops[0].dwEnd);
                            loop->count = U32(wh.Loops[0].dwPlayCount);
                        }
                        else
                        {
                            p->flags    = UC(0);
                            p->waveInfo.rawWave.loop=0;
                        }
                    }
                    else
                    {
...
char name[VAR_NAME_SIZE];
	...
	strcpy( name, val );
	p = (ALSound*) CURPTR;	
	add_2_list( SOUND, val, pos );		// Добавили запись в лист
	NEXT(sizeof(ALSound));				// Теперь указываем на обьект
...
loop:
	...
	if (IS("venvelope"))		
	{
		...
		p->venvelope	= PTR((ALEnvelope *)VPTCTL(ENVELOPE));
	}
	else if	(IS("penvelope"))		
	{	
		p->flags	   |= UC(AL_ENV_PAN);
		p->penvelope	= PTR((ALEnvelope *)VPTCTL(ENVELOPE));
	}	
	else if	(IS("keymap"))			p->keyMap		= PTR((ALKeyMap*)   VPTCTL(KEYMAP));
	else if	(IS("use"))				p->wavetable	= PTR((ALWaveTable*)VPTCTL(WAVE));
	else if	(IS("pan"))				p->samplePan	= VUC;
	else if	(IS("volume"))			p->sampleVolume	= VUC;
	else if	(IS("fadeout")) 		p->sampleFadeout= VS16;
	else if	(IS("end")) 			
	{
	...
	else    print_e( var,"not valid parameter for SOUND object");	
...
char name[VAR_NAME_SIZE];
	...
	p = (ALSound_*) CURPTR;	
	add_2_list( SOUND, val, pos );		// Добавили запись в лист
	NEXT(sizeof(ALSound_));				// Теперь указываем на обьект
...
loop:
	...
	if	(IS("envelope"))		
	{	
		p->flags	   |= UC(AL_ENV_VOL | AL_ENV_VOL_OLD);
		p->envelope		= PTR((ALEnvelope *)VPTCTL(ENVELOPEOLD));
	}	
	else if	(IS("keymap"))		p->keyMap		= PTR((ALKeyMap*)   VPTCTL(KEYMAP));
	else if	(IS("use"))			p->wavetable	= PTR((ALWaveTable*)VPTCTL(WAVE));
	else if	(IS("pan"))			p->samplePan	= VUC;
	...
	else if	(IS("end"))
	{
		if(p->keyMap == 0) print_e(name,"Object have no keyMap");
		if(p->wavetable == 0) print_e(name,"Object have no WaveTable");
		if(p->envelope == 0) print_e(name,"Object have no Envelope");
		return;
	}
	else    print_e( var,"not valid parameter for SOUND object");	
...
char name[VAR_NAME_SIZE];
	...
	add_2_list( INSTRUMENT, val, pos);	// Добавили запись в лист
	NEXT(sizeof(ALInstrument));		// Теперь указываем на обьект
...
loop:
	...
	else if	(IS("pan"))			p->pan			=VUC;
	else if	(IS("priority"))	p->priority		=VUC;
	else if	(IS("flags"))		p->flags		=VUC;
...
short instnum = 0;
	...
	// Увеличили число банков на один и создали ещё один банк
	bf = (ALBankFile*)ctl;
	bf->bankArray[0] = PTR((ALBank*)PTCTL(pos));
	
	p = (ALBank*)CURPTR; 
	add_2_list( BANK, val, pos );	// Добавили запись в лист
	NEXT(sizeof(ALBank));
    ...
    p->pad = 0;
#endif
    p->sampleRate = S32(44100);	/* e.g. 44100, 22050, etc...       */

loop:

	if(get_params(var,val,val1) == 0) return;

	     if (IS("flags"))		p->flags		=VSC;
	else if	(IS("samplerate"))	p->sampleRate	=VS32;
	else if	(IS("percussion"))	p->percussion	=PTR((ALInstrument*)VPTCTL(INSTRUMENT));
	else if	(IS("instrument"))
	{	
		p->instArray[instnum++] = PTR((ALInstrument*)VPTCTL(INSTRUMENT));	
		NEXT(sizeof(pctl));	
	}
	else if	(IS("end"))	
	{		
								p->instCount = S16(instnum); 	
								return;
	}
	else    print_e( var,"not valid parameter for BANK object");	
