...
#endif
	p->revision		= S16((unsigned short)(('B'<<8) + '2'));	// Версия файла
...
loop:
    ...
    {	p = (ALWaveTable*) CURPTR;  
        add_2_list( WAVE, val, pos );   // Добавили запись в лист
		NEXT(sizeof(ALWaveTable));                       // Теперь указываем на обьект sizeof(ALWaveTable)
...
loop:
	...
	else if	(IS("pan"))				p->samplePan	= VPAN;
...
loop:
	...
	if	(IS("percussion"))	p->percussion	=PTR((ALInstrument*)VPTCTL(INSTRUMENT));
	else if	(IS("instrument"))
	{	
		ins = s16val(val);
		if(maxins < ins)
		{	NEXT((ins-maxins) * sizeof(pctl));	
			for(;maxins<ins;maxins++) p->instArray[ins] = 0;
		}
